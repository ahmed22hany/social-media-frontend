...
const AdminProfile = () => {
  ...
  const handleDeletePost = async (postId) => {
    try {
      ...
      setAdminPost((prevPosts) => prevPosts.filter((post) => post._id !== postId));
    } catch (error) {
  ...
  const handleEditSubmit = async (post) => {
    try {
      await axios.put(`http://localhost:5001/api/post/${post._id}`, { ...editPostData });
      setIsEditing(null);
      setAdminPost(
        adminPost.map((p) =>
          p._id === post._id ? { ...p, ...editPostData } : p
        )
      );
    } catch (error) {
  ...
  return (
    ...
    <div className="admin-profile max-w-4xl mx-auto p-4">
      ...
      <ul className="space-y-6">
        {adminPost.map((post) => (
          <li key={post._id} className="bg-dark-2 text-white shadow rounded-lg p-4 relative mb-10"> {/* Added margin between posts */}
            {isEditing === post._id ? (
              <div>
                {/* Show textarea if the post is being edited */}
                <textarea
                  name="text"
              ...
              <div>
                {/* Show post content if it's not being edited */}
            ...
            {/* Replies Section */}
            <div className="mt-6 border-t border-gray-300 pt-4"> {/* Separated replies from post with border */}
              <h4 className="font-semibold">Replies</h4>
              <ul className="space-y-2 mt-2">
                {post.replies.map((reply) => (
                  <li key={reply._id} className="bg-dark-2 text-white p-2 rounded-lg border border-gray-200">
                    <p>{reply.text}</p>
                  </li>
                ))}
              </ul>
...
//       <ul className="space-y-6">
//         {adminPost.map((post) => (
//           <li key={post._id} className="bg-dark-2 text-white shadow rounded-lg p-4 relative mb-10">
//             {isEditing === post._id ? (
//               <div>
//                 <textarea
//                   name="text"
...
//                 {post.replies.map((reply) => (
//                   <li key={reply._id} className="bg-dark-4 p-2 text-white">
//                     <p >{reply.text}</p>
//                     <div className="absolute top-2 right-2 flex space-x-2">
//                     </div>
//                   </li>
